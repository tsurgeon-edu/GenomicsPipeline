...
class PreProcessing(object):
    ...
    def mark_duplicate(self, merged_bam, chr):
        ...
        if self.split_chr == "After":
            ...
            output = os.path.join(
                os.path.dirname(merged_bam),
                mark_prefix_removed + "_" + os.path.basename(merged_bam)
            )
            marked_dup_metrics = os.path.join(
                os.path.dirname(merged_bam),
                "marked_dup_metrics" + chr[:-4] + ".txt"
            )
            picardcommand = helpers.picard_cmd(
                f"MarkDuplicates I={merged_bam} O={output} M={marked_dup_metrics} "
            ...
            marked_dup_metrics = os.path.join(os.path.dirname(merged_bam), "marked_dup_metrics.txt")
            picardcommand = helpers.picard_cmd(
                f"MarkDuplicates I={merged_bam} O={output} M={marked_dup_metrics} "
                f"REMOVE_DUPLICATES=true CREATE_INDEX=true"
            )
